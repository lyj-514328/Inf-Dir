...
- Web viewer 的第三方库（如 foliate-js）以 submodule 固定，页面与解析适配器等集成改动提交在该 fork 内；**必须先把 submodule 提交 push 到远端，再把父仓库 gitlink 指过去**，禁止记录只存在于本地的 submodule 提交（否则全新克隆 `git submodule update --init` 会报 `upload-pack: not our ref`）
