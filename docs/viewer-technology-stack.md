...
| 图片与 RAW | `img-view` | 已接入 | Rust 原生解码；SVG 用 resvg；RAW 经 LibRaw decoder → Windows WIC decoder → ImageMagick 顺序处理；manifest 扩展名已与系统 WIC 注册格式对齐（HEIF/AVIF、JPEG XL、WebP、Raw 相机格式及 `.icon` `.jfif` `.dib` 等别名） | `image`、`resvg`；可选 LibRaw decoder、ImageMagick、Compface、Windows WIC |
...
| `inf-dir.image-view` | Rust + egui/eframe | image、resvg | 可选 LibRaw decoder、ImageMagick、Compface、WIC decoder | 常用位图、SVG、相机 RAW、专业图像格式（HEIF/AVIF、JPEG XL、WebP、JPEG-XR 由系统 WIC 兜底） |
...
| Windows WIC decoder | `img-view` | `img-view/wic-decoder/wic-decoder.exe`；LibRaw decoder 之后的第三级解码回退，覆盖系统注册的 HEIF/AVIF、JPEG XL、WebP 与 Raw 相机格式；`--list` 可枚举本机解码器 |
| LibRaw decoder | `img-view` | `img-view/libraw-decoder/`（`libraw-decoder.exe` + `libraw.dll`，LibRaw 0.22.2 Win64 官方包）；RAW 首选解码器，输出 PPM 流 |
