...
| EPUB | `ebook-view`、`mupdf-view` | 已接入，两个候选 | foliate-js 直接排版，或 MuPDF.NET 直接打开 | WebView2；`ebook-view-web/` 静态资源；MuPDF.NET |
| MOBI | `ebook-view`、`mupdf-view` | 已接入，两个候选 | foliate-js `mobi.js`（内容嗅探 MOBI/KF8），或 MuPDF.NET 直接打开 | WebView2；`ebook-view-web/` 静态资源；MuPDF.NET |
| FB2 | `ebook-view`、`mupdf-view` | 已接入，两个候选 | foliate-js `fb2.js`，或 MuPDF.NET 直接打开 | WebView2；`ebook-view-web/` 静态资源；MuPDF.NET |
| FBZ / FB2Z | `ebook-view`、`mupdf-view` | 已接入（转换） | `.fbz` 由 foliate-js 页面内解压（`view.js` 的 `isFBZ`）；`.fb2z` 只有 `mupdf-view` 命中，从 ZIP 中提取 FB2 再交给 MuPDF.NET | foliate-js 内置 zip.js；.NET `System.IO.Compression`；MuPDF.NET |
| TCR | `ebook-view`、`mupdf-view` | 已接入（转换） | 解压 TCR 字典格式为临时 HTML，再交给 foliate-js 的 `html-book.js` 或 MuPDF.NET | .NET / Rust 内置字节解析；MuPDF.NET |
| CBZ | `ebook-view`、`mupdf-view`、`archive-view` | 已接入，三个候选 | foliate-js `comic-book.js` 渲染；MuPDF 直接读取 ZIP 漫画；archive-view 可列出归档内容 | WebView2；MuPDF.NET；libarchive |
| CBR | `ebook-view`、`mupdf-view`、`archive-view` | 已接入（转换） | `ebook-view` / `mupdf-view` 启动旁边的 `archive-view.exe --extract-comic`，用 libarchive 解包图片，按自然序重新打包临时 CBZ 后交给 foliate-js 或 MuPDF | `archive-view.exe` + `archive.dll`；MuPDF.NET；无需新增 RAR 二进制 |
| DjVu / DJV | `ebook-view`、`mupdf-view` | 已接入，两个候选 | DjVuLibre `ddjvu.exe` 转 PDF，再交给 foliate-js 自带的 pdf.js 或 MuPDF.NET | 共享 DjVuLibre 运行时（`inf-dir.runtime/`） |
| XPS / OXPS | `ebook-view` | 已接入（转换） | 共享运行时里的 GhostXPS `gxpswin64.exe -sDEVICE=pdfwrite` 转 PDF，再交给 foliate-js 自带的 pdf.js | 共享 GhostXPS 运行时（`inf-dir.runtime/`） |
...
| `inf-dir.ebook-view` | Rust + winit/wry/WebView2 | foliate-js fork（新增 HTML book 适配） | WebView2；随包发布的 `ebook-view-web/` 静态资源（submodule = fork）；DjVu 与 XPS/OXPS 转换分别用共享的 DjVuLibre、GhostXPS 运行时 | EPUB、MOBI、AZW/AZW3、FB2/FBZ、CBZ、DjVu、TCR、CBR、XPS/OXPS |
...
| DjVuLibre | `mupdf-view`、`ebook-view` | 共享运行时 `inf-dir.runtime/djvulibre/`，核心是 `ddjvu.exe` |
| GhostXPS 10.08.0 | `ebook-view` | 共享运行时 `inf-dir.runtime/gxps/`，用 `gxpswin64.exe -sDEVICE=pdfwrite` 把 XPS/OpenXPS 转 PDF；内含 `gxpsdll64.dll` 与全部许可文件，随包的示例 `.xps` 不发布 |
| LibreDWG | `mupdf-view` | `mupdf-view/libredwg/`，包含 `dwg2SVG.exe` / `dxf2dwg.exe` |
| LibreOffice 26.2.5 | `mupdf-view` | 共享运行时 `inf-dir.runtime/libreoffice/`，调用 `program/soffice.exe`；压缩包不是上游发布，按 SHA-256 固定 |
...
- **XPS 转换运行时的选型记录**（2026-09）：GhostXPS 10.08.0，取自 Artifex 挂在
  ...
  抽不出可用文本层。`mutool` 唯一的独有优势是还能接管未来下线 `mupdf-view` 时的其余转换
  格式，届时可按需重新从 `mupdf-<版本>-windows.zip` 取回（同一套共享运行时解析顺序，
  无需改代码）。
...
- `ebook-view (foliate-js)` 的扩展格式已接入，转换全部在 Rust 启动层完成：
  ...
  - **XPS / OXPS**：调用共享运行时里的
    `gxpswin64.exe -q -dNOPAUSE -dBATCH -sDEVICE=pdfwrite -o<临时>.pdf <输入>`，把矢量
    页面转成 PDF，再交给 foliate-js 自带的 `pdf.js`。简单文本型 `.xps` 的文本层可正常
    抽取；复杂的阿拉伯文 `.oxps` 两边（GhostXPS 与 MuPDF）都抽不出可用文本层。
