...
（DjVuLibre：`ebook-view` 把 DjVu 转成 PDF；GhostXPS：`pdfjs-view` 把 XPS/OpenXPS 转成
PDF；LibreOffice：`pdfjs-view` 把 Office/ODF/CAD 转成 PDF，`excel-view` 把
`xls/xlt/xlsb/ods/ots` 转成 xlsx，`web-view` 把 Visio 转成 SVG）。它没有 `plugin.json`，
